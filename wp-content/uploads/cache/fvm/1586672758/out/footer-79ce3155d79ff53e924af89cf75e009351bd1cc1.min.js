/* https://coradashboard.com/wp-includes/js/imagesloaded.min.js */
try{
...
/* https://coradashboard.com/wp-includes/js/masonry.min.js */
try{
/*! This file is auto-generated */
/*!
...
/* https://coradashboard.com/wp-includes/js/clipboard.min.js */
try{
/*! This file is auto-generated */
...
/* https://coradashboard.com/wp-content/plugins/betterdocs/public/js/betterdocs-public.js */
try{
(function($){"use strict";$(document).ready(function(){var request;var searchForm=$("#betterdocs-searchform");var searchField=$("#betterdocs-search-field");var searchClose=$(".docs-search-close");searchForm.on("keyup keypress",function(e){var keyCode=e.keyCode||e.which;if(keyCode===13){e.preventDefault();return!1}});searchField.on("keyup",function(e){var inputVal=$(this).val();var resultWrapper=$(".betterdocs-live-search");var resultList=$(".betterdocs-live-search .betterdocs-search-result-wrap");var searchLoader=$(".docs-search-loader");if(e.key!="a"&&e.keyCode!=17&&e.keyCode!=91&&inputVal.length>=1){delay(function(){ajaxLoad(inputVal,resultWrapper,resultList,searchLoader,searchClose)},300)}else if(!inputVal.length){$(".betterdocs-live-search .betterdocs-search-result-wrap").addClass("hideArrow");$(".betterdocs-live-search .docs-search-result").slideUp(300);searchLoader.hide();searchClose.hide()}});searchClose.on("click",function(){$(this).hide();$(".betterdocs-live-search .betterdocs-search-result-wrap").addClass("hideArrow");$(".betterdocs-live-search .docs-search-result").slideUp(300);searchField.val("")});var delay=(function(){var timer=0;return function(callback,ms){clearTimeout(timer);timer=setTimeout(callback,ms)}})();function ajaxLoad(inputVal,resultWrapper,resultList,searchLoader,searchClose){if(request){request.abort()}
request=$.ajax({url:betterdocspublic.ajax_url,type:"post",data:{action:"betterdocs_get_search_result",search_input:inputVal},beforeSend:function(){searchLoader.show();searchClose.hide();resultList.addClass("hideArrow");$(".betterdocs-live-search .docs-search-result").slideUp(400)},success:function(html){resultList.remove();searchLoader.hide();searchClose.show();resultWrapper.append(html)}})}
var betterdocsToc=$(".betterdocs-entry-content .betterdocs-toc");if(betterdocsToc.length){var stickyTocContent=$(".betterdocs-toc").clone();$(".sticky-toc-container").append(stickyTocContent);$(window).on("scroll",function(){var betterdocsSidebar=$('#betterdocs-sidebar');var stickyToc=$(".sticky-toc-container");var tocHeight=$(".betterdocs-sidebar-content").outerHeight();var tocSidebar=document.querySelector(".betterdocs-sidebar-content");var tocSidebarRect=tocSidebar.getBoundingClientRect();var tocSidebarTop=Math.abs(tocSidebarRect.top);if(tocSidebarRect.top<0&&tocHeight<=tocSidebarTop){stickyToc.addClass("toc-sticky")}else{stickyToc.removeClass("toc-sticky")}
if($(window).scrollTop()>=betterdocsSidebar.offset().top+betterdocsSidebar.outerHeight()-window.innerHeight){stickyToc.removeClass("toc-sticky")}})}
$(document).on("scroll",onScroll);var toc_links=$(".betterdocs-toc .toc-list a");toc_links.on("click",function(e){e.preventDefault();$(document).off("scroll");toc_links.each(function(){$(this).removeClass("active")});$(this).addClass("active");var target=this.hash,$target=$(target);$("html, body").stop().animate({scrollTop:$target.offset().top},600,"swing",function(){window.location.hash=target;$(document).on("scroll",onScroll)})});function onScroll(){var scrollPos=$(document).scrollTop();$(".sticky-toc-container .betterdocs-toc .toc-list a,.layout2-toc-container .betterdocs-toc .toc-list a").each(function(){var currLink=$(this);var refElement=$(currLink.attr("href"));if(refElement.position().top<=scrollPos&&refElement.position().top+refElement.height()>scrollPos){$(".betterdocs-toc .toc-list a").removeClass("active");currLink.addClass("active")}})}
$(".close-toc").on("click",function(event){event.preventDefault();$(".sticky-toc-container").remove(".sticky-toc-container")});$('body').on("click",".betterdocs-print-btn",function(event){var entryTitle=document.getElementById('betterdocs-entry-title').innerHTML;var printContents=document.getElementById('betterdocs-single-content').innerHTML;var combined=document.createElement('div');combined.innerHTML='<h1>'+entryTitle+'</h1>'+" "+printContents;combined.id='new-doc-print';var pwidth=document.getElementById("betterdocs-single-content").offsetWidth;var wheight=$(window).height();var winPrint=window.open('','','left=50%,top=10%,width='+pwidth+',height='+wheight+',toolbar=0,scrollbars=0,status=0');winPrint.document.write(combined.outerHTML);winPrint.document.close();winPrint.focus();winPrint.print();winPrint.close()});var sidebarContent=$(".betterdocs-sidebar-content");var catList=$(".betterdocs-sidebar-content .docs-single-cat-wrap .docs-item-container");var currentCatList=$(".betterdocs-sidebar-content .docs-single-cat-wrap.current-category .docs-item-container");var catHeading=$(".betterdocs-sidebar-content .docs-single-cat-wrap .docs-cat-title-wrap");catList.hide();if(currentCatList.length){currentCatList.show().addClass("show")}
catHeading.click(function(){var $this=$(this);sidebarContent.find(".active-title").removeClass("active-title");$this.toggleClass("active-title");if($this.next(catList).hasClass("show")){$this.next(catList).slideUp().removeClass("show")}else if(catList.hasClass("show")){catList.slideUp().removeClass("show");$this.next(catList).slideToggle().toggleClass("show")}else{$this.next(catList).slideToggle().toggleClass("show")}});var docSubCat=$('.docs-sub-cat-title');docSubCat.each(function(){$(this).click(function(e){e.preventDefault();$(this).children('.toggle-arrow').toggle();$(this).next('.docs-sub-cat').fadeToggle(0)})})
var width=$(window).width();if(width<768){var docTocTitle=$('.betterdocs-entry-content .collapsible-sm .toc-title');docTocTitle.each(function(){$(this).click(function(e){e.preventDefault();$('.betterdocs-entry-content .collapsible-sm').children('.angle-icon').toggle();$(this).next('.toc-list').slideToggle()})})}
var formModal=$("#betterdocs-form-modal");var formModalContent=$("#betterdocs-form-modal .modal-content");$("a[name=betterdocs-form-modal]").click(function(e){e.preventDefault();formModal.fadeIn(500)});$(document).mouseup(function(e){if(!formModalContent.is(e.target)&&formModalContent.has(e.target).length===0){formModal.fadeOut()}});$(".betterdocs-modalwindow .close").click(function(e){e.preventDefault();formModal.fadeOut(500)});var feedbackForm=$("#betterdocs-feedback-form");var feedbackFormFields=$("#betterdocs-feedback-form input, #betterdocs-feedback-form textarea");feedbackFormFields.on("keyup",function(){$(this).removeClass("val-error");$(this).siblings(".error-message").remove()});feedbackForm.on("submit",function(e){e.preventDefault();var form=$(this);var message_name=$("#message_name");var message_email=$("#message_email");var message_subject=$("#message_subject");var message_text=$("#message_text");betterdocsFeedbackFormSubmit(form,message_name,message_email,message_subject,message_text)});function betterdocsFeedbackFormSubmit(form,message_name,message_email,message_subject,message_text){if(request){request.abort()}
request=$.ajax({url:betterdocspublic.ajax_url,type:"post",data:{action:"betterdocs_feedback_form_submit",form:form.serializeArray(),message_name:message_name.val(),message_email:message_email.val(),message_subject:message_subject.val(),message_text:message_text.val()},beforeSend:function(){},success:function(data){var data=JSON.parse(data);if(data.sentStatus){if(data.sentStatus==="success"){$(".response").html('<span class="success-message">'+data.sentMessage+"</span>");form[0].reset();delay(function(){$(".betterdocs-modalwindow").fadeOut(500)},3000)}else{$(".response").html('<span class="error-message">'+data.sentMessage+"</span>")}}else{if(data.nameStatus==="error"){if(message_name.hasClass("val-error")==!1){message_name.addClass("val-error");$(".form-name").append('<span class="error-message">'+data.nameMessage+"</span>")}}
...
$(".anchor").hover(function(){var title=$(this).attr("data-title");$('<div/>',{text:title,class:'tooltip-box'}).appendTo(this)},function(){}).on('click',function(e){e.preventDefault();var a=new ClipboardJS('.anchor');a.on('success',function(e){$(document).find("div.tooltip-box").text('Copied');e.clearSelection(),$(e.trigger).addClass("copied"),setTimeout(function(){$(e.trigger).removeClass("copied")},2000)})});(function(){if(typeof self==='undefined'||!self.Prism||!self.document){return}
...
/* https://coradashboard.com/wp-content/plugins/betterdocs/public/js/simplebar.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/easy-digital-downloads/assets/js/jquery.creditCardValidator.min.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/easy-digital-downloads/assets/js/edd-checkout-global.min.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/easy-digital-downloads/assets/js/edd-ajax.min.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/pp-edd-fastspring/assets/js/main.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/wp-smushit/app/assets/js/smush-lazy-load.min.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/pp-edd-fastspring/assets/js/checkout.js */
try{
var ppeddfsFormClass='pp-edd-fs-checkout-form';(function($){fastspring.builder.reset();if(''!==PPEDDFS_CHECKOUT.payload&&''!==PPEDDFS_CHECKOUT.key){fastspring.builder.secure(PPEDDFS_CHECKOUT.payload,PPEDDFS_CHECKOUT.key)}
$(document).on('edd_gateway_loaded',function(){if(0!=PPEDDFS_CHECKOUT.total){$('#edd_checkout_user_info').hide();$('#edd_checkout_user_info input').removeAttr('required');$('#edd_purchase_form').addClass(ppeddfsFormClass)}
$('#edd_payment_mode_select').remove();$('#edd_cc_address').remove();$('#edd_cc_fields').remove();$('#edd_checkout_login_register').remove();$('.edd_cart_subtotal_row').remove();$('.edd_cart_tax_row').remove()});$(document).on('submit','.'+ppeddfsFormClass,function(event){event.preventDefault();fastspring.builder.checkout()});$(window).on('load',function(){$(document).trigger('edd_gateway_loaded')});$(document).on('edd_discount_applied',function(event,discountResponse){if(0==discountResponse.total_plain){$('#edd_purchase_form').removeClass(ppeddfsFormClass);$('#edd_checkout_user_info').slideDown();$('#edd_checkout_user_info input[name="edd_email"], #edd_checkout_user_info input[name="edd_first"]').prop('required',!0);return!1}
$('.edd_cart_amount').hide();window.location.reload(!0)});$(document).on('edd_discount_removed',function(){$('.edd_cart_amount').hide();window.location.reload(!0)});$(document).on('edd_quantity_updated',function(){$('.edd_cart_amount').hide();window.location.reload(!0)});$('.js-pp-edd-cart-item-price').each(function(){$(this).siblings('.edd_cart_item_price').remove();$(this).addClass('edd_cart_item_price')});$('.js-pp-edd-cart-total').each(function(){$(this).siblings('.edd_cart_total').remove()});$('.js-pp-edd-cart-fee-price').each(function(){$(this).siblings('.edd_cart_fee_amount').remove();$(this).siblings('.edd_cart_fee_label').after($(this).detach())})})(jQuery);function eddCheckoutResetPurchaseButton(){var $submitButton=jQuery('.'+ppeddfsFormClass).find('#edd-purchase-button');$submitButton.val(PPEDDFS_CHECKOUT.checkout_button_text);$submitButton.prop('disabled',!1);$submitButton.siblings('.edd-loading').remove()};

}
catch(e){console.error("An error has occurred: "+e.stack);}
/* https://coradashboard.com/wp-includes/js/wp-embed.min.js */
try{
/*! This file is auto-generated */
...
/* https://coradashboard.com/wp-content/plugins/elementor/assets/js/frontend-modules.min.js */
try{
...
/* https://coradashboard.com/wp-includes/js/jquery/ui/position.min.js */
try{
/*!
...
/* https://coradashboard.com/wp-content/plugins/elementor/assets/lib/dialog/dialog.min.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/elementor/assets/lib/waypoints/waypoints.min.js */
try{
...
/* https://coradashboard.com/wp-content/plugins/elementor/assets/lib/swiper/swiper.min.js */
try{
/**
 * Swiper 5.3.6
...
/* https://coradashboard.com/wp-content/plugins/elementor/assets/lib/share-link/share-link.min.js */
try{
